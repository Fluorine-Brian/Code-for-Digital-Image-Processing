...
def pad_image_for_dft(image, padding_mode='reflect'):
    ...
    # np.pad takes (before, after) for each dimension
...
def create_gaussian_lowpass_filter(shape, D0):
    ...
    # Apply GLPF formula
    # Avoid division by zero if D0 is 0 (though D0 should be > 0 for a meaningful filter)
    if D0 == 0:
...
if __name__ == "__main__":
    ...
    # Image filename to process (Fig 4.44(a) is the same as Fig 4.40(a))
    ...
    # These D0 values are used directly for GLPF.
    ...
    # --- Visualization (Matching Fig 4.44(a) to (f)) ---
    ...
    # Subplot 1: Original Image (Fig 4.44(a))
    plt.subplot(2, 3, 1)
    plt.imshow(original_image, cmap='gray', vmin=0, vmax=255)
    plt.title('a) Original Image')
    plt.axis('off')

    # Subplots 2-6: Filtered Images (Fig 4.44(b) to (f))
